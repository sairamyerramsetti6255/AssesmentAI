...
import { generateAssessmentQuestions, generateExpectedAnswer, generateResearchNotes, clearGenericBenchmarks, } from '../lib/assessmentAi.js';
...
function buildQuestionRecords(assessmentId, generated) {
...
function resolvePainPointIds(names) {
    return names
 ...
 */
router.post('/generate-all', requireRole('super_admin', 'sales_manager'), async (req, res) => {
    const assessmentId = req.params.id;
    ...
    try {
        ...
        if (msg.includes('Complete Step 1') || msg.includes('company name'))
...
router.post('/generate-questions', requireRole('super_admin', 'sales_manager'), async (req, res) => {
    const assessmentId = req.params.id;
    const assessment = demoStore.assessments.find((a) => a.id === assessmentId);
    if (!assessment)
        return res.status(404).json({ error: 'Not found' });
...
router.post('/generate-all-benchmarks', requireRole('super_admin', 'sales_manager'), async (req, res) => {
    const assessmentId = req.params.id;
    const questions = demoStore.questions.filter((q) => q.assessment_id === assessmentId && q.session_status !== 'deleted');
    if (!questions.length)
        return res.status(400).json({ error: 'Generate questions first' });
    const results = [];
...
router.post('/clear-generic-benchmarks', requireRole('super_admin', 'sales_manager'), async (req, res) => {
    const assessmentId = req.params.id;
    const cleared = clearGenericBenchmarks(assessmentId);
    res.json({ cleared });
});
router.post('/questions/:qid/generate-expected-answer', requireRole('super_admin', 'sales_manager'), async (req, res) => {
    const assessmentId = req.params.id;
...
router.post('/questions', requireRole('super_admin', 'sales_manager'), async (req, res) => {
    const assessmentId = req.params.id;
    const { question_text, question_type = 'rating', options, driver_id, is_required = true, rating_min = 1, rating_max = 5, expected_answer_time_seconds = 60, } = req.body;
...
router.delete('/questions/:qid', requireRole('super_admin', 'sales_manager'), async (req, res) => {
    const assessmentId = req.params.id;
    const questionId = req.params.qid;
    const question = demoStore.questions.find((q) => q.id === questionId && q.assessment_id === assessmentId);
    if (!question)
...
router.put('/questions', requireRole('super_admin', 'sales_manager'), async (req, res) => {
    const assessmentId = req.params.id;
