...
router.post('/', requireRole('super_admin', 'sales_manager'), async (req, res) => {
    ...
    if (isGeminiConfigured() && score) {
        try {
            const parsed = await geminiGenerateJSON(`Given AI readiness scores ${JSON.stringify(driverScores)} for ${industry?.name || 'a company'} (${client?.company_name || 'client'}), identify gaps and recommend from: ${demoStore.masters.solutions.map((s) => s.solution_name).join(', ')}. Return JSON: {gaps: [{driver, gap, severity}], recommended_solutions: [{solution_name, solution_key, rationale}]}`);
            if (parsed.gaps)
                gaps.splice(0, gaps.length, ...parsed.gaps);
            if (parsed.recommended_solutions)
                recommended.splice(0, recommended.length, ...parsed.recommended_solutions);
        }
    ...
    if (idx >= 0)
        demoStore.gapAnalyses[idx] = analysis;
    else
