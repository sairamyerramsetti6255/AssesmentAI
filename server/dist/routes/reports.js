...
router.get('/overview', async (req, res) => {
    ...
    for (const a of assessments) {
        ...
        if (!byIndustry[name])
            byIndustry[name] = { count: 0, avgScore: 0 };
        byIndustry[name].count++;
        const score = scores.find((s) => s.assessment_id === a.id);
        if (score)
            byIndustry[name].avgScore += score.overall_score;
    }
    Object.keys(byIndustry).forEach((k) => {
        if (byIndustry[k].count)
            byIndustry[k].avgScore = Math.round(byIndustry[k].avgScore / byIndustry[k].count * 100) / 100;
    });
