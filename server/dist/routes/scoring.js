...
async function computeAndPersistScore(assessmentId, userId) {
    ...
    if (existing >= 0)
        demoStore.scores[existing] = score;
    else
    ...
    if (assessment?.created_by) {
        await createNotification(assessment.created_by, 'Scores Ready', 'AI session analysis and scoring is complete', 'success', `/assessments/${assessmentId}/results`);
    }
    return score;
}
router.post('/score', async (req, res) => {
    ...
    try {
        ...
        if (!score)
            return res.status(400).json({ error: 'No session data to score' });
        res.json(score);
    }
    catch (err) {
