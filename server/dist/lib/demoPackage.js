...
export function ensureDemoScore(assessmentId) {
    ...
    const benchmark = assessment?.industry_benchmark_snapshot;
    const driver_scores = {};
    ...
    const overall = Math.round((Object.values(driver_scores).reduce((a, b) => a + b, 0) / demoStore.masters.drivers.length) * 10) / 10;
    ...
    const score = {
        ...
        benchmark_comparison: {
            ...
            delta: Object.fromEntries(Object.keys(driver_scores).map((k) => [
                k,
...
export function ensureDemoGap(assessmentId) {
...
export function ensureDemoPoc(assessmentId) {
...
export function ensureDemoProposal(assessmentId) {
    ...
    html = html
        ...
        .replace(/\{\{poc_section\}\}/g, `<p>See the formal PoC letter prepared for this engagement. Key title: ${poc.content.title || 'Proof of Concept Plan'}.</p>`);
    const proposal = {
        id: uuidv4(),
        assessment_id: assessmentId,
    ...
    if (idx >= 0)
        demoStore.proposals[idx] = proposal;
    else
        demoStore.proposals.push(proposal);
    return proposal;
}
export function buildDemoPackage(assessmentId) {
    const score = ensureDemoScore(assessmentId);
    const gap = ensureDemoGap(assessmentId);
    const poc = ensureDemoPoc(assessmentId);
