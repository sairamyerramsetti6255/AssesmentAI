...
function formatAnswerDisplay(answer, otherText, richtext) {
    if (richtext?.trim())
...
export async function syncClientResponseRows(input) {
    ...
    const rows = questions.map((q) => {
        ...
        return {
            ...
            taxonomy_pillar: q.taxonomy_pillar ?? '',
    ...
    try {
        ...
        const withAnswers = rows.filter((r) => r.answer_display !== '—' || r.answer_json !== null);
...
export async function getClientResponseRows(leadId) {
    try {
