...
export function saveLeadDocumentFile(
  leadId: string,
  docId: string,
  originalName: string,
  buffer: Buffer,
...
export function readLeadDocumentFile(storagePath: string): Buffer | null {
  const abs = path.join(ROOT, storagePath)
  if (!fs.existsSync(abs)) return null
  return fs.readFileSync(abs)
}

export function demoDocumentContent(name: string, transactionDate: string): Buffer {
  const body = [
