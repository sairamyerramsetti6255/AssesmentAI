...
router.put('/:id/answers', requireRole('super_admin', 'sales_rep'), async (req: Request, res: Response) => {
...
router.patch('/:id/questions/:qid', requireRole('super_admin', 'sales_rep'), async (req: Request, res: Response) => {
  ...
  if (action === 'modify' && question_text) {
    if (!question.original_question_text) question.original_question_text = question.question_text;
    question.question_text = question_text;
  } else if (action === 'skip') {
    question.session_status = 'skipped';
    question.skip_reason = skip_reason || 'Skipped by rep';
  } else if (action === 'delete') {
    if (question.is_required) return res.status(400).json({ error: 'Cannot delete required question' });
...
router.post('/:id/recordings', requireRole('super_admin', 'sales_rep'), upload.single('audio'), async (req: Request, res: Response) => {
...
router.post('/:id/transcribe', requireRole('super_admin', 'sales_rep'), async (req: Request, res: Response) => {
  const sessionId = param(req.params.id);
  ...
  if (recording) recording.transcript = transcript;
...
router.post('/:id/complete', requireRole('super_admin', 'sales_rep'), async (req: Request, res: Response) => {
  const sessionId = param(req.params.id);
  const session = demoStore.sessions.find((s) => s.id === sessionId);
  if (!session) return res.status(404).json({ error: 'Not found' });
