...
**Env (required — without Neon, login returns 503):**
```
...
Set `DATABASE_URL` to the Neon connection string (direct host, not the pooler, if the pooler drops idle connections). Paste it into Coolify **Environment Variables**. Do **not** commit `server/.env`.
